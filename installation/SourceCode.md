...
#### Примение изменений, переменные будут устаналиваться каждый раз при логине пользователя.
```
. ~/.profile
```
...
#### страницы будут снабжены контрольными суммами и Postgres будет проверять, что контрольная сумма совпадает - спасает от дисковых проблем
```
initdb -k
```
...
### Останов сервера, pg_ctl stop по сути останавливает процесс postmaster
...
pg_ctl status
```
или посмотреть в файле, в первой строке номер процесса
```
